...
class RealSenseAbstractDeprojector_old : public core::objectmodel::BaseObject {
public :
    SOFA_CLASS( RealSenseAbstractDeprojector_old , core::objectmodel::BaseObject);
        ...
        , m_pointcloud(new pcl::PointCloud<pcl::PointXYZ>)
    {
        ...
        c_intrinsics.addCallback(std::bind(&RealSenseAbstractDeprojector_old::readIntrinsics, this));
        c_scale.addInputs({&d_scale, &d_tr_offset}) ;
        c_scale.addCallback(std::bind(&RealSenseAbstractDeprojector_old::deproject_image, this));
        c_projmat.addInputs({&d_projectionmatrix}) ;
        c_projmat.addCallback(std::bind(&RealSenseAbstractDeprojector_old::updateRotation, this)) ;
    }
    ...
    void readIntrinsics () {
        if (l_rs_cam) {
            std::cout << "(RealSenseprojector_olds) link to realsense cam is valid, " <<
        ...
        std::cout << "(RealSenseprojector_olds) successfully read intrinsics from file " <<
...
//        << std::endl ;
    ...
    inline void deproject_image_offline () {
        ...
        if (diststruct._width != (size_t)(depth_im.cols/downSample) ||
            ...
            //std::cerr << "(deprojector_old) check sizes" << std::endl ;
            return ;
        }
